...
void addChild(Tree *T, Node parent_index, Node child_index) {
    if (parent_index < 0 || parent_index >= MAX_NODES ||
        ...
        fprintf(stderr, "addChild: index out of bounds (parent=%d child=%d)\n",
    ...
    if (!newChild) {
        perror("addChild: malloc failed");
        exit(EXIT_FAILURE);
    }
    newChild->childIndex = child_index;
    newChild->next = T->children[parent_index]; // insert at head
    ...
    for (int i = 0; i < MAX_NODES; i++) {
        if (T->children[i] == NULL) {
            ...
            for (int j = 0; j < MAX_NODES; j++) {
                ChildNode *curr = T->children[j];
                while (curr != NULL) {
                    if (curr->childIndex == i) {
                        existsInTree = true;
                        break;
                    }
                    curr = curr->next;
                }
                if (existsInTree) break;
            }
            if (!existsInTree && i != T->rootIndex) continue;

            if (minLeaf == INVALID_NODE || i < minLeaf) {
                minLeaf = i;
            }
        }
    }
...
Node right(const Tree *T) {
    ...
    for (int i = 0; i < MAX_NODES; i++) {
        if (T->children[i] == NULL) {
            ...
            if (maxLeaf == INVALID_NODE || i > maxLeaf) {
                maxLeaf = i;
            }
        }
    }
